---
title: "An example of numerical instability with Wilkinson's polynomial"
author: "JMA"
...
sensitive test for different numerical encodings (e.g. IEEE 785?), as we will demonstrate with encodings for web transport. 
...
```{r}
for (z in 12:19 )
    print(sapply(1:z, function(x) evaluate.wilkinson(x, 1+z)))
```
...
for (z in 12:19 )
    print(sapply(1:z, function(x) evaluate.wilkinson.json(x, 1+z)))
```
