...
### Wilkinson's polynomial, a numerical computation horror story

With Halloween just around the corner, it's the season to raise the 
the horrors that occur when the
...
Google's _Protocol Buffers_ convert language-specific data structures to a universal binary transfer format. It comes with interfaces in several languages, but not `R`. Fortunately on CRAN there is a package `RProtoBuf` wraps _Protocol Buffers_, and goes a step further, to generate protocol payload conversion descriptions dynamically (that take the place of writing a `.proto` definition file), hiding some of the machinery needed with other languages. 
